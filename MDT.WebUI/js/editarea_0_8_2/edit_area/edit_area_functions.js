	...
	EditArea.prototype.open_file= function(settings){
		...
		if(settings['id']!="undefined")
		{
			...
			if(!_$(this.files[id]['html_id']) && id!="")
			{
				...
				//var close = "<img src=\"" + parent.editAreaLoader.baseURL + "images/close.gif\" title=\"" + this.get_translation('close_tab', 'word') + "\" onclick=\"if(!confirm('关闭本标签并不保存数据，是否确定？')){return;};editArea.execCommand('close_file', editArea.filesIdAssoc['" + html_id + "']);return false;\" class=\"hidden\" onmouseover=\"this.className=''\" onmouseout=\"this.className='hidden'\" />";
				//elem.innerHTML= "<a style=\"height:25px;\" onclick=\"javascript:editArea.execCommand('switch_to_file', editArea.filesIdAssoc['"+ html_id +"']);\" selec=\"none\"><b><span><strong class=\"edited\">*</strong>"+ this.files[id]['title'] + close +"</span></b></a>";
				elem.innerHTML = "<a style=\"height:25px;\" onclick=\"javascript:editArea.execCommand('switch_to_file', editArea.filesIdAssoc['" + html_id + "']);\" selec=\"none\"><b><span><strong class=\"edited\">*</strong>&nbsp;&nbsp;&nbsp;&nbsp;" + this.files[id]['title'] + "&nbsp;&nbsp;&nbsp;&nbsp;</span></b></a>";
